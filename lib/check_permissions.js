...
function checkPermissions (args, cb) {
  ...
  console.log('tested:', args.msg.role, cmd );
  ...
  function checkValidity(profile, validityCb) {
    ...
    function applyCustomValidator (done) {
      ...
      async.every(customValidator, function(validatorAct, validatorCb) {
        console.log('customValidator',  _.defaults(validatorAct, _.omit(_.clone(args.msg), omittedFields)));
        var msg = _.defaults(validatorAct, _.omit(_.clone(args.msg), omittedFields));
